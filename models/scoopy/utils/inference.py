...
from models.SCOOP.models.scoop import SCOOP
from models.SCOOP.models.refiner import Refiner
from datasets.generic import Batch
from .seed import seed_everything
from .utils import log_string, iterate_in_chunks
...
def eval_model(scoop, testloader, log_file, log_dir, res_dir, args):
    ...
    for it, batch in enumerate(tqdm(testloader)):
        ...
        # print(testloader.dataset.filename_curr)
        # Send data to GPU
        batch = batch.to(device, non_blocking=True)
        batch.data["fname"] = fname
        # Estimate flow
        ...
        if args.use_test_time_refinement:
            ...
            refined_flow, refine_metrics_curr, duration_curr, KNN, normals = refiner.refine_flow(batch, est_flow.detach(), corr_conf.detach(), test_time_optimizer, args)
        ...
        if save_pc_res:
            ...
            fname_pc_save = ".".join([fname_split[0] + "_res", 'npz'])
            path_pc_save = os.path.join(res_dir, fname_pc_save)
            np.savez(path_pc_save, pc1=pc1_save, pc2=pc2_save, gt_mask_for_pc1=mask_save, gt_flow_for_pc1=gt_save, est_flow_for_pc1=est_flow_save, corr_conf_for_pc1=corr_conf_save, KNN=KNN.detach().cpu().numpy(), normals=normals.detach().cpu().numpy())
...
def my_main(args):
    ...
    pathroot = os.path.join(f"{os.path.expanduser('~')}/data/")
    # Select dataset
    if args.dataset_name.split("_")[0].lower() == "HPLFlowNet".lower():

        # HPLFlowNet version of the datasets
        path2data = os.path.join(pathroot, "HPLFlowNet")
    ...
    # Load Checkpoint
    file = torch.load(args.path2ckpt)#, map_location='cpu')
    ...
    name_of_exp = args.exp_name
    log_dir = os.path.expanduser("~") + "/experiments/" + name_of_exp
    os.makedirs(log_dir, exist_ok=True) # later not use exist_ok!!!
    ...
    mean_epe, _,_,_ = eval_model(scoop, testloader, log_file, log_dir, res_dir, args)

    print(f"____________ \n"
          f"KNN: {args.nb_neigh_smooth_flow} \n"
          f"Visibility: {args.use_visibility_smooth_loss} \n"
          f"Normals: {args.use_normals_smooth_loss} \n"
          f"Mean EPE: {mean_epe:.4f} \n"
          f"____________ \n")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_name", type=str, default="HPLFlowNet_kitti", help="Dataset. FlowNet3D_kitti or FlowNet3D_FT3D or Either HPLFlowNet_kitti or HPLFlowNet_FT3D.")
    ...
    parser.add_argument("--use_visibility_smooth_loss", type=int, default=0, help="1: Use visibility-aware self smooth flow loss in test time refinement, 0: Do not use visibility smooth flow loss.")  # can be more specific to avoid nested args
    parser.add_argument("--use_normals_smooth_loss", type=int, default=0, help="1: Use normal-aware self smooth flow loss in test time refinement, 0: Do not use visibility smooth flow loss.")  # can be more specific to avoid nested args
    ...
    parser.add_argument("--exp_name", type=str, default='example', help="Name of experiment, destination of experiment folder.")
    parser.add_argument("--path2ckpt", type=str, default=f"./pretrained_models/kitti_v_100_examples/model_e400.tar", help="Path to saved checkpoint.")
    parser.add_argument("--log_fname", type=str, default="log_evaluation.txt", help="Evaluation log file name.")
    parser.add_argument("--save_pc_res", type=int, default=1, help="1: save point cloud results, 0: do not save point cloud results [default: 0]")
    parser.add_argument("--res_folder", type=str, default="pc_res", help="Folder name for saving results.")
    parser.add_argument("--save_metrics", type=int, default=1, help="1: save evaluation metrics results, 0: do not save evaluation metrics results [default: 0]")
