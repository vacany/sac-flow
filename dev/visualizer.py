...
class LaserScanVis:
  ...
  def __init__(self, scan, scan_names=[1,2,3], label_names=None, offset=0,
  ...
  def update_scan(self):
    ...
    range_data = np.linalg.norm(points, axis=1)
    ...
    # self.scan_vis.set_data(points,)
    vispy.scene.visuals.Arrow(pos=points, color='red', parent=self.scan_vis)
    ...
    if self.images:
      ...
      # plot range image
      image = np.random.randint(0, 255, (64, 1024))
      data = np.copy(image)
...
scan = np.random.rand(1000, 3)
LaserScanVis(scan).run()
