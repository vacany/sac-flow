...
class Batch:
    def __init__(self, batch):
        ...
        self.data['indices'] = batch[0]['indices']
...
class SceneFlowDataset(Dataset):    # here augment as well?
    def __init__(self, nb_points, all_points=True):
        """
    ...
    def __getitem__(self, idx):
        # sequence, ground_truth, orig_size, indices = self.to_torch(*self.subsample_points_rnd(*self.load_sequence(idx)))
        data = self.load_sequence(idx)
        ...
        for key in data.keys():
            ...
            if 'pc' in key:
                pc_list.append(data[key])

            if 'inst_pc' in key:
                inst_list.append(data[key])

            if 'gt_flow' in key:
                gt_flow_list.append(data[key])

            if 'gt_mask' in key:
                gt_mask_list.append(data[key])

            if 'depth' in key:
                depth_list.append(data[key])


        data_frame = {"pcs": pc_list, "gt_flow": gt_flow_list, "gt_mask": gt_mask_list, "inst_pc": inst_list, "depth": depth_list, 'frame_id': data['frame_id']}

        return data_frame

    def instance_to_torch(self, np_array):
        return torch.from_numpy(np_array).unsqueeze(0)

    def to_torch(self, sequence, ground_truth, orig_size, indices):
        """
        ...
        indices = [torch.unsqueeze(torch.from_numpy(i), 0) for i in indices]

        return sequence, ground_truth, orig_size, indices
    ...
    def subsample_points_rnd(self, sequence, ground_truth):
        ...
        # print(idx1_perm.shape)
        indices = [idx1_perm, idx2_perm]
        return sequence, ground_truth, orig_size, indices
