...
data_path = f'{DATA_PATH}/sceneflow/ft3d/'

lidar_pose = (0,0,0)

all_files = sorted(glob.glob(data_path + '/TEST*.npz'))
test_idx = range(len(all_files))
...
# kittit
# ['flow', 'color1', 'color2', 'points2', 'points1', 'valid_mask1']
def remap_keys(data_npz):

    data_npz = dict(data_npz)

    data_npz['pc1'] = data_npz['points1'].copy()
    data_npz['pc2'] = data_npz['points2'].copy()
    data_npz['gt_flow'] = data_npz['flow'].copy()
    data_npz['id_mask1'] = data_npz['valid_mask1'].copy()
    data_npz['color1'] = data_npz['color1'].copy()
    del data_npz['points1']
    del data_npz['points2']
    del data_npz['flow']
    del data_npz['valid_mask1']
